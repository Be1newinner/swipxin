...
export function VideoCall({
    ...
    import('../lib/socket.js').then(({ default: socketService }) => {
      ...
      const rawMatchFoundListener = async (data) => {
        ...
        try {
          ...
          console.log(`🚑 [${user.id}] FORCING immediate transition to video chat`);
          ...
          console.log(`📦 [${user.id}] Emergency match object created:`, emergencyMatch);
          ...
          console.log(`🎉 [${user.id}] State updates complete, showing toast...`);
          toast.success(`Connected with ${data.partner?.name || 'your match'}!`);
          
          console.log(`✅ [${user.id}] Emergency transition completed successfully`);
          
        } catch (error) {
          console.error(`❌ [${user.id}] Error during emergency transition:`, error);
  ...
  const handleFindMatch = async () => {
    if (user.tokens < 1) {
      toast.error('Insufficient tokens. You need at least 1 token to start a video call.');
      navigateTo('wallet');
      return;
    }
    ...
    try {
      ...
      matchingService.subscribeToMatches({
        onMatchFound: async (match) => {
          console.log(`✅ [${user.id}] Match found via real-time subscription:`, match);
          ...
          console.log(`🎉 [${user.id}] Transitioning to video chat with:`, formattedMatch);
        ...
        onMessageReceived: (message) => {
          console.log(`💬 [${user.id}] Message received via real-time:`, message);
      ...
      try {
        ...
        if (match) {
          console.log(`✅ [${user.id}] Match found directly from promise:`, match);
          ...
          setIsInVideoChat(true);
          setIsSearchingMatch(false);
          toast.success(`Connected with ${match.partner?.name || 'your match'}!`);
        }
      } catch (matchError) {
  ...
  if (isInVideoChat && currentMatch) {
    console.log(`🎥 [${user.id}] Rendering VideoChat component with match:`, currentMatch);
    return (
      <VideoChat
        match={currentMatch}
        currentUser={user}
          ...
          <Card className="glass p-6 text-center">
            {isSearchingMatch ? (
              ...
              <div className="space-y-4">
                ...
                <div>
                  <h3 className="font-semibold text-lg">Finding your match...</h3>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm">
                  ...
                  <p className="text-amber-700 dark:text-amber-300 mt-1">
                    You need at least 1 token to start a video chat. Purchase more tokens to continue.
                  </p>
                  <Button
                    variant="link"
                    size="sm"
