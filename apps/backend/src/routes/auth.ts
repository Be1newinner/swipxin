  ...
  try {
    ...
    await query(
      ...
      [
        ...
        "deduction",
        0,
        "Free account created - upgrade to premium for tokens!",
      ]
    );
...
router.post("/login", loginValidation, async (req, res) => {
  try {
    ...
    if (user.is_premium && user.tokens <= 0) {
      ...
      console.log(
        `📉 User ${user.name} (${user.email}) downgraded to FREE - 0 tokens remaining`
      );
      ...
      await query(
        ...
        [
          ...
          "deduction",
          0,
          "Downgraded to FREE - premium tokens exhausted",
        ]
      );
    }
    ...
    try {
      ...
      const planDetails = {
        basic: {
          tokens: 100,
          price: 9.99,
          description: "Basic Plan - 100 tokens",
        },
        pro: {
          tokens: 300,
          price: 24.99,
          description: "Pro Plan - 300 tokens",
        },
        unlimited: {
          tokens: 1000,
          price: 49.99,
          description: "Unlimited Plan - 1000 tokens",
        },
      };
      ...
      console.log(
        `💎 User ${result.rows[0].name} upgraded to PREMIUM (${plan}) - ${selectedPlan.tokens} tokens added`
      );
  ...
  try {
    ...
    await query(
      ...
      [
        ...
        "call",
        -tokenCost,
        "Video call connection - 8 tokens deducted",
      ]
    );
    ...
    if (remainingTokens <= 0) {
      ...
      await query(
        ...
        [
          ...
          "downgrade",
          0,
          "Auto-downgraded to FREE - tokens exhausted",
        ]
      );
