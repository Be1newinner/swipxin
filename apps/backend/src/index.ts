...
io.on("connection", async (socket) => {
  ...
  socket.on("joinMatchingQueue", async (preferences = {}) => {
    try {
      console.log(
        `\ud83d\udd0d User ${socket.user.name} joined matching queue with preferences:`,
        preferences
      );
      ...
      console.log(
        `\ud83d\udcca Queue size after adding ${socket.user.name}: ${waitingUsers.size}`
      );
      ...
      if (!matchFound && waitingUsers.size >= 2) {
        console.log(
          `\ud83d\udd04 No immediate match found, processing entire queue...`
        );
      ...
      if (waitingUsers.size >= 2) {
        setTimeout(async () => {
          console.log(
            `\ud83d\udd0d Auto-triggering match processing for ${waitingUsers.size} users...`
          );
          await processMatchingQueue();
  ...
  socket.on("joinVideoRoom", (data) => {
    ...
    console.log(
      `\ud83d\udcf9 User ${socket.user.name} joined video room ${roomId}`
    );
  });
  ...
  socket.on("leaveVideoRoom", async (data) => {
    ...
    console.log(
      `\ud83d\udeaa User ${socket.user.name} left video room ${roomId}`
    );
  });
  ...
  socket.on("sendMessage", async (data) => {
    try {
      ...
      if (matchResult.rows.length > 0) {
        ...
        console.log(
          `\ud83d\udcac Message sent in match ${matchId} by ${socket.user.name}`
        );
      }
    } catch (error) {
  ...
  socket.on("updateOnlineStatus", async (data) => {
    try {
      ...
      if (userId !== socket.userId) {
        console.warn(
          `User ${socket.userId} tried to update status for user ${userId}`
        );
        return;
      }
      ...
      console.log(
        `🟢 Updated online status for user ${socket.user.name}: ${isOnline}`
      );
...
async function findMatch(userId: string) {
  try {
    ...
    console.log(
      `🔍 Looking for match for ${user.name}. ${waitingUsers.size} users in queue.`
    );
    console.log(
      `📋 Current queue:`,
      Array.from(waitingUsers.keys()).map((id) => {
    ...
    if (matchedUserId && matchedUserData) {
      console.log(
        `✅ Confirmed match: ${user.name} <-> ${matchedUserData.user.name}`
      );
      ...
      console.log(
        `🗑️ Removed both users from queue. Remaining: ${waitingUsers.size}`
      );
      ...
      const user1SocketExists =
        user1Socket && io.sockets.sockets.has(user1Socket.socketId);
      const user2SocketExists =
        user2Socket && io.sockets.sockets.has(user2Socket.socketId);

      console.log(
        `🔍 Socket validation - User1: ${user1SocketExists ? "✅" : "❌"}, User2: ${user2SocketExists ? "✅" : "❌"}`
      );
      ...
      if (user1SocketExists && user2SocketExists) {
        ...
        console.log(
          `📤 Sending match notification to ${user.name} (${user1Socket.socketId})`
        );
        console.log(
          `📤 Sending match notification to ${matchedUserData.user.name} (${user2Socket.socketId})`
        );
        ...
        console.log(
          `🎉 Match notifications sent: ${user.name} <-> ${matchedUserData.user.name} (Room: ${roomId})`
        );
        ...
        console.log(
          `❌ Invalid socket connections - User1Socket: ${user1Socket?.socketId}, User2Socket: ${user2Socket?.socketId}`
        );
      ...
      console.log(
        `⏳ No available match for ${user.name}. Staying in queue with ${waitingUsers.size} total users.`
      );
...
async function processMatchingQueue() {
  try {
    ...
    for (let i = 0; i < userIds.length; i++) {
      ...
      for (let j = i + 1; j < userIds.length; j++) {
        ...
        if (
          user1Data &&
          user2Data &&
          activeUsers.has(userId1) &&
          activeUsers.has(userId2) &&
          io.sockets.sockets.has(user1Data.socketId) &&
          io.sockets.sockets.has(user2Data.socketId)
        ) {
          console.log(
            `🎯 Processing queue match: ${user1Data.user.name} <-> ${user2Data.user.name}`
          );
          ...
          if (user1Socket && user2Socket) {
            ...
            console.log(
              `🎉 Queue match created: ${user1Data.user.name} <-> ${user2Data.user.name}`
            );
            matchesFound++;
    ...
    console.log(
      `✅ Queue processing complete. ${matchesFound} matches created. ${waitingUsers.size} users remaining.`
    );
  } catch (error) {
...
setInterval(async () => {
  if (waitingUsers.size >= 2) {
    console.log(
      `🔄 Periodic matching check: ${waitingUsers.size} users waiting`
    );
    await processMatchingQueue();
  }
